...
void LookAndFeel::drawRotarySlider(juce::Graphics& g,
    ...
    g.setColour(enabled ? Colours::crimson : Colours::darkgrey);
    g.fillEllipse(bounds);

    g.setColour(enabled ? Colours::bisque : Colours::grey);
    ...
    if (auto* rswl = dynamic_cast<RotarySliderWithLabels*>(&slider)) {
        ...
        float norm = slider.valueToProportionOfLength(slider.getValue());

        auto sliderAngRad = jmap(norm, 0.f, 1.f, rotaryStartAngle, rotaryEndAngle);
    ...
    while (leftChannelFifo->getNumCompleteBuffersAvailable() > 0) {
        if (leftChannelFifo->getAudioBuffer(tempIncomingBuffer)) {
            ...
            juce::FloatVectorOperations::copy(monoBuffer.getWritePointer(0, monoBuffer.getNumSamples() - size),
                tempIncomingBuffer.getWritePointer(0, 0),
                size);
...
void ResponseCurveComponent::updateChain() {
    ...
    auto peakCoeficcients = makePeakFilter(chainSettings, audioProcessor.getSampleRate());
    updateCoefficients(monoChain.get<ChainPositions::Peak>().coefficients, peakCoeficcients);
